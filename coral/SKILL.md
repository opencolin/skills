...
3. **Page or narrow discovery instead of dumping full catalogs.** If you need more tables, repeat the `coral.tables` query with `OFFSET 50`, `OFFSET 100`, and so on. Prefer exact `schema_name` filters and table-name substrings when you know the source or topic.
...
For large tables, page column metadata with `OFFSET 100`, `OFFSET 200`, and so on, or narrow with predicates such as `column_name LIKE '%<term>%'` or `is_required_filter = true`.
